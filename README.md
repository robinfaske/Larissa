...
*Views are my own; all data is free and public (Banxico SIE, BCB/ANBIMA,
treasury.gov, Bundesbank). Nothing here reflects any employer.*
...
*(Figures are generated by the run below; they appear after the first
populated run — see status note.)*
...
Code, tests, and conventions are complete; `data/cache/` is not yet
populated because this repo was built in a sandbox that blocks financial
data hosts (see the environment note at the top of
[DECISIONS.md](DECISIONS.md) — nothing was fabricated to fill the gap).
First `--refresh` on an open network populates the cache; commit those CSVs
and the repo reproduces offline from then on.
